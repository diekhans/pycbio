...
* add pretty_print bases on this
import sys
from collections import namedtuple

def _pretty_print_recurse(name, value, indent, level, fh):
    print(' ' * (indent * (level + 1)) + f"{name}=", end="", file=fh)
    pretty_print(value, indent, level+1, inline=True, fh=fh)

def pretty_print(obj, indent=4, level=0, *, inline=False, fh=sys.stderr):
    """
    ...
    if hasattr(obj, "_fields"):
        ...
        for field in obj._fields:
            _pretty_print_recurse(field, getattr(obj, field), indent, level, fh)
        print(space + ")", file=fh)
    elif isinstance(obj, (list, tuple)):
        print(startsp + "[", file=fh)
        for item in obj:
            pretty_print(item, indent, level + 1, fh=fh)
        print(space + "]", file=fh)
    elif isinstance(obj, dict):
        print(startsp + "{", file=fh)
        for key, value in obj.items():
            _pretty_print_recurse(repr(key), value, indent, level, fh)
        print(space + "}", file=fh)
    elif hasattr(obj, "__dict__") or hasattr(obj, "__slots__"):
        print(startsp + f"{obj.__class__.__name__}(", file=fh)
        for key, value in vars(obj).items():
            _pretty_print_recurse(key, value, indent, level, fh)
        print(space + ")", file=fh)
    else:
        print(startsp + repr(obj), file=fh)
2
# Define namedtuple
Bag = namedtuple("Bag", ("one", "two", "three", "four"))
...
data = [
    Bag(
        "First element",
        ["Nested list", "Another item"],
        {"key1": "value1", "key2": ["list in dict", 123]},
        42,
    ),
    Box(
        "First element",
