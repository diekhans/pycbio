...
// A select filter's choices are built on the page (_colSpec.selectValues), so the
// list is the column's own values and the sort order is decided there.  A cell with
// no value is picked by a SENTINEL choice, not by "": Tabulator reads an empty
// header-filter value as no filter at all, so an empty string cannot stand for "the
// rows with nothing here".  A control character cannot collide with cell text.
var _SELECT_EMPTY = "\u0000";
function _selectChoices(spec) {
    var choices = spec.selectValues.map(function(v) { return {label: v, value: v}; });
    if ("emptyLabel" in spec) {
        choices.unshift({label: spec.emptyLabel, value: _SELECT_EMPTY});
    }
    return choices;
}
function _selectMatch(text, choice) {
    return (choice === _SELECT_EMPTY) ? (text === "") : (text === choice);
}
// several choices are an OR within the column; across columns the filters still AND,
// which is Tabulator's own behavior
function _selectFilter(headerValue, rowValue, rowData, params) {
    var text = ("" + (rowData[params.field] == null ? "" : rowData[params.field])).trim();
    var chosen = Array.isArray(headerValue) ? headerValue : [headerValue];
    return chosen.some(function(choice) { return _selectMatch(text, choice); });
}
// multiselect hands back an array, so nothing picked is an empty one; a cleared
// single value comes back null or ""
function _selectEmpty(value) {
...
function _fitColumnWidths() {
    ...
    fits.forEach(function(c) {
        ...
        // a pull-down puts its arrow, and the picked values, in the header cell
        if (c.filter === "select") need = Math.max(need, 90);
...
var _columns = _colSpec.map(function(c) {
    ...
    if (_opts.headerFilters && c.filter !== "none") {
        if (c.filter === "range") {
            ...
            col.headerFilterParams = {values: _selectChoices(c), multiselect: true,
                                      clearable: true};
            col.headerFilterFunc = _selectFilter;
            col.headerFilterFuncParams = {field: c.textField};
            col.headerFilterEmptyCheck = _selectEmpty;
        } else {
...
// the row the frameset opened on (openFirstRow), so it is highlighted without
// having been clicked; null when the browser frame opened on the default
var _currentId = ("currentId" in _opts) ? _opts.currentId : null;
