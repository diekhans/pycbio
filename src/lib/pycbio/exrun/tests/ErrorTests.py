...
from pycbio.exrun.Graph import Rule,GraphException, CycleException
...
class GraphTests(TestCaseBase):
    ...
    def testCycle(self):
        ...
        try:
            ...
            er.run()
        except GraphException, ex:
            self.failUnlessEqual(str(ex), "Production ErrorTests.GraphTests.testCycle.file2 already linked to it's produces, attempt to add: rule3")
        if ex == None:
            self.fail("expected GraphException")
    ...
    def testNoRule(self):
        ...
        try:
            ...
            self.failUnlessEqual(str(ex),"no rule to build: ErrorTests.GraphTests.testNoRule.file3")
        if ex == None:
