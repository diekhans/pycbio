...
def testCigars():
    ...
    for cigarStr in cases:
        ...
        assert cigarStr == str(cigar)

def testExonerateSpaceCigars():
...
def testMinimapCigar():
    ...
    for cigarStr in cases:
        cigar = cigarStringParse(cigarStr)
        assert cigarStr == cigar.format(inclOnes=True)

def testPysamToCigar(request):
    with (pysam.AlignmentFile(ts.get_test_input_file(request, "ont-rna.sam"), "r") as samfh,
          open(ts.get_test_output_file(request, ".cigar"), 'w') as cigarfh):
        for aln in samfh:
            cigar = cigarFromPysam(aln.cigartuples)
            print(str(cigar), file=cigarfh)
    ts.diff_results_expected(request, ".cigar")
