...
def testAssemblyReport(request):
    asmReport = AssemblyReport(ts.get_test_input_file(request, "GCF_000001405.28.assembly.txt"))
    seqLines = [str(seq) for seq in asmReport.seqs]
    assert seqLines == ['1\tassembled-molecule\t1\tChromosome\tCM000663.2\t=\tNC_000001.11\tPrimary Assembly\t249698942\tchr1',
    ...
    assert str(asmReport.getBySequenceName('HSCHR19KIR_T7526_BDEL_HAP_CTG3_1')) == expectLine
    assert str(asmReport.getByGenBankAccn('KI270916.1')) == expectLine
    assert str(asmReport.getByRefSeqAccn('NT_187670.1')) == expectLine
    assert str(asmReport.getByUcscStyleName('chr19_KI270916v1_alt')) == expectLine

    for name in ('HSCHR19KIR_T7526_BDEL_HAP_CTG3_1', 'KI270916.1', 'NT_187670.1', 'chr19_KI270916v1_alt'):
        assert str(asmReport.getByName(name)) == expectLine
    ...
    assert metaData == [('assembly_level', 'Chromosome'),
                        ...
                        ('taxid', '9606')]
    assert asmReport.assemblyName == 'GRCh38.p2'
    with pytest.raises(AssemblyReportNotFound, match="^sequence 'fred' not found in NCBI AssemblyReport for 'GRCh38.p2'$"):
        asmReport.getByName("fred")

def testAssemblyReportNewMeta(request):
    # updated metadata name capitalization
    asmReport = AssemblyReport(ts.get_test_input_file(request, "GCF_000001405.39_GRCh38.p13_assembly_report.txt"))
    assert asmReport.assemblyName == 'GRCh38.p13'
    metaData = sorted(list(asmReport.metaData.items()))
    assert metaData == [('assembly_level', 'Chromosome'),
                        ...
                        ('synonyms', 'hg38'),
                        ('taxid', '9606')]
    with pytest.raises(AssemblyReportNotFound, match="^sequence 'fred' not found in NCBI AssemblyReport for 'GRCh38.p13'$"):
        asmReport.getByName("fred")

def testAgpGrch38(request):
    agp = Agp(ts.get_test_input_file(request, "GCA_000001305.2.chr22.agp"))
    agpRows = [rec.format(True) for rec in agp.recs]
    assert agpRows == ['CM000684.2\t1\t10000\t1\tN\t10000\ttelomere\tno\tna',
...
def testAgpClint(request):
    agp = Agp(ts.get_test_input_file(request, "GCA_002880755.3.chr10.agp"))
    agpRows = [rec.format(True) for rec in agp.recs]
    assert agpRows == ['CM009248.2\t1\t19390040\t1\tW\tKZ622779.1\t1\t19390040\t-',
